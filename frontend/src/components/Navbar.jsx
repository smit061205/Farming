...
export default function Navbar({ activeLink = "dashboard" }) {
  ...
  return (
    <nav
      className="fixed top-0 left-0 right-0 z-50 backdrop-blur-xl rounded-b-[3rem] shadow-[0_10px_30px_rgba(23,56,9,0.04)]"
      style={{ backgroundColor: "rgba(254,250,224,0.7)" }}
    >
      <div className="flex justify-between items-center w-full px-12 py-6 max-w-[1920px] mx-auto gap-8">
        <div className="flex-1">
          <Link
            to="/"
            className="text-2xl font-black text-[#173809] uppercase tracking-tighter font-headline hover:opacity-80 transition-opacity"
          >
        ...
        <div className="hidden md:flex gap-12 items-center">
          {[
            ...
            { label: "Library", path: "/soil-library" },
        ...
        <div
          className="flex flex-1 justify-end items-center gap-4 relative"
          ref={menuRef}
        >
          {/* Render target for Google Translate */}
          <div
            ref={translateContainerRef}
            className="translate-wrapper flex items-center justify-center"
          ></div>
          ...
          {token ? (
            ...
            <Link
              to="/login"
              className="notranslate bg-[#173809] text-white px-5 py-2 rounded-full font-headline font-bold text-sm hover:bg-[#2d4f1e] active:scale-95 transition-all outline-none shadow-md whitespace-nowrap"
            >
              Sign In
            </Link>
          )}
