...
Ported from the reference Node server's lib/gemini.js. Two things this
handles that a naive request does not:
  1. Model fallback. Flash models return 503 under load and retired names
     404. We walk a chain rather than failing the farmer's request.
  2. Token budget. Flash models spend output tokens on internal reasoning,
